    ...
    for (let i = 0; i < dataTypes.length; i++) { 
        ...
        for (let e = 0; e < dataTypes[i].double_damage_from.length; e++) {
            if (arrayDoubleDamageFrom.length > 0) {                                
                if (arrayDoubleDamageFrom.find(x => x.type.name === dataTypes[i].double_damage_from[e].name) === undefined) {
                    ...
                    let newDamage = arrayDoubleDamageFrom.find(x => x.type.name === dataTypes[i].double_damage_from[e].name);
        ...
        for (let e = 0; e < dataTypes[i].double_damage_to.length; e++) {
            if (arrayDoubleDamageTo.length > 0) {                                
                if (arrayDoubleDamageTo.find(x => x.type.name === dataTypes[i].double_damage_to[e].name) === undefined) {
                    ...
                    let newDamage = arrayDoubleDamageTo.find(x => x.type.name === dataTypes[i].double_damage_to[e].name);
                    newDamage.type.damage = 2*2;
                }
            } else {
                arrayDoubleDamageTo.push(
        ...
        for (let e = 0; e < dataTypes[i].half_damage_from.length; e++) {
            if (arrayHalfDamageFrom.length > 0) {                                
                if (arrayHalfDamageFrom.find(x => x.type.name === dataTypes[i].half_damage_from[e].name) === undefined) {
                    ...
                    let newDamage = arrayHalfDamageFrom.find(x => x.type.name === dataTypes[i].half_damage_from[e].name);
        ...
        for (let e = 0; e < dataTypes[i].half_damage_to.length; e++) {
            if (arrayHalfDamageTo.length > 0) {                                
                if (arrayHalfDamageTo.find(x => x.type.name === dataTypes[i].half_damage_to[e].name) === undefined) {
                    ...
                    let newDamage = arrayHalfDamageTo.find(x => x.type.name === dataTypes[i].half_damage_to[e].name);
                    newDamage.type.damage = 0.5*0.5;
                }
            } else {
                arrayHalfDamageTo.push(
        ...
        for (let e = 0; e < dataTypes[i].no_damage_from.length; e++) {
            if (arrayNoDamageFrom.length > 0) {                                
                if (arrayNoDamageFrom.find(x => x.type.name === dataTypes[i].no_damage_from[e].name) === undefined) {
                    ...
                    let newDamage = arrayNoDamageFrom.find(x => x.type.name === dataTypes[i].no_damage_from[e].name);
        ...
        for (let e = 0; e < dataTypes[i].no_damage_to.length; e++) {
            if (arrayNoDamageTo.length > 0) {                                
                if (arrayNoDamageTo.find(x => x.type.name === dataTypes[i].no_damage_to[e].name) === undefined) {
                    ...
                    let newDamage = arrayNoDamageTo.find(x => x.type.name === dataTypes[i].no_damage_to[e].name);
                    newDamage.type.damage = 0*0;
                }
            } else {
                arrayNoDamageTo.push(
    ...
    if (dataTypes.length > 1) {
        ...
        if (arrayNoDamageFrom.length > 0) {            
            for (let i = 0; i < arrayNoDamageFrom.length; i++) {
                ...
                if(arrayHalfDamageFrom.find(x => x.type.name === arrayNoDamageFrom[i].type.name) !== undefined) {
                    arrayHalfDamageFrom = arrayHalfDamageFrom.filter(type => type.type.name !== arrayNoDamageFrom[i].type.name);
                }                
            }

            for (let i = 0; i < arrayNoDamageFrom.length; i++) {

                if(arrayDoubleDamageFrom.find(x => x.type.name === arrayNoDamageFrom[i].type.name) !== undefined) {                    
                    arrayDoubleDamageFrom = arrayDoubleDamageFrom.filter(type => type.type.name !== arrayNoDamageFrom[i].type.name);                    
                }                
            }
        }
        ...
        if (arrayNoDamageTo.length > 0) {            
            for (let i = 0; i < arrayNoDamageTo.length; i++) {
                ...
                if(arrayHalfDamageTo.find(x => x.type.name === arrayNoDamageTo[i].type.name) !== undefined) {
                    arrayHalfDamageTo = arrayHalfDamageTo.filter(type => type.type.name !== arrayNoDamageTo[i].type.name);
                }                
            }

            for (let i = 0; i < arrayNoDamageTo.length; i++) {

                if(arrayDoubleDamageTo.find(x => x.type.name === arrayNoDamageTo[i].type.name) !== undefined) {                    
                    arrayDoubleDamageTo = arrayDoubleDamageTo.filter(type => type.type.name !== arrayNoDamageTo[i].type.name);                    
                }                
            }
        }
        ...
        if (arrayHalfDamageFrom.length > 0) { 
            ...
            for (let i = 0; i < arrayHalfDamageFrom.length; i++) {
                ...
                if(arrayDoubleDamageFrom.find(x => x.type.name === arrayHalfDamageFrom[i].type.name) !== undefined) {
                    arrayDoubleDamageFrom = arrayDoubleDamageFrom.filter(type => type.type.name !== arrayHalfDamageFrom[i].type.name); 
                    typesRemoved.push(arrayHalfDamageFrom[i].type.name);                    
                }                
            }

            for (let i = 0; i < typesRemoved.length; i++) {                
                arrayHalfDamageFrom = arrayHalfDamageFrom.filter(type => type.type.name !== typesRemoved[i]);                
            }
        }
        ...
        if (arrayHalfDamageTo.length > 0) { 
            ...
            for (let i = 0; i < arrayHalfDamageTo.length; i++) {
                ...
                if(arrayDoubleDamageTo.find(x => x.type.name === arrayHalfDamageTo[i].type.name) !== undefined) {
                    arrayDoubleDamageTo = arrayDoubleDamageTo.filter(type => type.type.name !== arrayHalfDamageTo[i].type.name); 
                    typesRemoved.push(arrayHalfDamageTo[i].type.name);                    
                }                
            }

            for (let i = 0; i < typesRemoved.length; i++) {                
                arrayHalfDamageTo = arrayHalfDamageTo.filter(type => type.type.name !== typesRemoved[i]);                
            }
        }
                ...
                <div className="table-responsive-sm table-responsive-md table-responsive-lg">
                    <table className="table table-bordered">
                        ...
                        <tbody>
                            <tr>
                                <td>Not effective</td>
                                <td><PokeTypes types={arrayNoDamageTo} /></td>
                                <td><PokeTypes types={arrayNoDamageFrom} /></td>                                    
                            </tr>

                            <tr>
                                <td>Not very effective</td>
                                <td><PokeTypes types={arrayHalfDamageTo} /></td>
                                <td><PokeTypes types={arrayHalfDamageFrom} /></td>                                   
                            </tr>

                            <tr>
                                <td>Super effective</td>
                                <td><PokeTypes types={arrayDoubleDamageTo} /></td>
                                <td><PokeTypes types={arrayDoubleDamageFrom} /></td>                                 
                            </tr>
                        </tbody>
                    </table>
                </div>
